...
| `employee_targets` **(new, Phase 11 §7.8 — shipped 2026-08-08)** | target_id, employee_id, target_type, target_month, target_year, assigned_target, achieved_target, status, created_by, created_at, updated_at | Per-employee monthly targets (currently `target_type='admission'` only). `achieved_target` in the sheet is a **cache only** — every GET recomputes it live from `admission_leads` (status = `Admission Completed`, matched by `assigned_to` + `updated_at` falling in the target's month/year — a documented proxy, not stakeholder-confirmed, since `admission_leads` has no dedicated "completed in month X" column). `assigned_target` defaults to the `MONTHLY_ADMISSION_TARGET` setting if left blank on assignment. No `branch_id` column — branch scoping for `branch_manager` is done via the employee's own `branch_id` + `canAccessBranch()`. Sheet created via one-time `setupEmployeeTargetsSheet()` — ⚠️ not yet confirmed run on the live Spreadsheet. |
...
| `appscript_employee_targets.gs` **(new, Phase 11 §7.8 — backend pre-dates 2026-08-08, frontend shipped 2026-08-08)** | `employee_targets` sheet — `getEmployeeTargets` (GET, role-filtered same pattern as `getDailyTasks`; live-recomputes `achieved_target`/`remaining_target`/`progress_percent` on every call), `assignTarget` (POST, super_admin/hr only, blocks duplicate active target for same employee+type+month+year), `updateTarget` (POST, edits `assigned_target`/`status` only), one-time `setupEmployeeTargetsSheet()`. `getEmployeeTargets` added to `EMPLOYEE_ALLOWED_GET_ACTIONS` (own-data-only, enforced in-handler). |
...
| **Employee Targets (Phase 11 §7.8)** | **`getEmployeeTargets`** (`?emp_id=`, `?month=`, `?year=`, `?target_type=`, `?status=` filters; role/branch-scoped, employee sees only own) | **`assignTarget`** (super_admin/hr only), **`updateTarget`** (super_admin/hr only) |
...
| `targets_admin.html` **(new, Phase 11 §7.8 — shipped 2026-08-08)** | super_admin / hr / branch_manager | Assign/edit Employee Targets — stats row, employee/month/year/status filters, assign+edit modal, per-row live progress bar. Same HR-standalone/admin-sidebar dual-mode pattern as `daily_tasks_admin.html`/`admissions.html`. Reachable via `sidebar.html` + `hr_dashboard.html` navbar/quick-action (both already wired, added when the page didn't exist yet). |
...
*This README reflects the state of the codebase as of the latest changes (Phase 11: Settings sheet, 30-day leave lock, Employee leave summary, Daily Tasks module §7.5–§7.7 fully shipped incl. admin approval UI `daily_tasks_admin.html` — 2026-08-02; Admission Leads module §7.10/§7.11 fully shipped + verified live, incl. `admission_leads` sheet, `appscript_admissions.gs`, and `admissions.html` — 2026-08-03; `holidays.html` NaN date-chip bug fixed — 2026-08-08; Employee Targets §7.8 (`targets_admin.html`) and Reports §7.9 (`reports.html`) frontends shipped, `employees.html` §7.1 display-name bug fixed — 2026-08-08; §7.1 fully closed 2026-08-08 — `appscript_employees.gs`'s `getEmployees` confirmed by direct code review to attach `department_name`/`designation_name`/`branch_name` via `resolveEmployeeDisplayNamesBulk()`, no remaining backend-wiring gap). Keep it updated as new modules are added.*