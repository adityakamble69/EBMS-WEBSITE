...
- [ ] `daily_tasks` sheet + employee add/list/self-view routes (§7.5) — ⚠️ **backend shipped 2026-08-02** (`appscript_daily_tasks.gs`: `getDailyTasks`/`addDailyTask`/`updateDailyTask`, `setupDailyTasksSheet()`), **frontend not started** — employee submit form + §7.7 calendar view still needed before this box can be checked.
- [ ] Daily task admin approval flow + routes (§7.6) — ⚠️ **backend shipped 2026-08-02** (`approveDailyTask`/`rejectDailyTask`/`requestCorrectionDailyTask`), **frontend not started** — admin-side approval UI still needed before this box can be checked.
