...
Assign targets per employee/month from Settings or target data — never hardcode target values

Calculate Achieved from completed admission leads (status = Admission Completed)

Show Assigned / Achieved / Remaining / Progress % on the employee dashboard

HR Dashboard month-wise reports (§7.9)

Filters: month, year, department, employee, report type

Report types: attendance, leave, daily task, target performance, job candidate, admission, joining
...
Fix the pre-existing employees.html employee-profile display-name issue noted under §7.1
