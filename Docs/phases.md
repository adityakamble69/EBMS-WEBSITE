...
HR target tracking (`hr_targets` sheet, Assigned/Achieved/Remaining/Progress %) (§7.8) — ⚠️ **scope-corrected and rebuilt 2026-08-08**: the original build (2026-08-08, same day, earlier session) wrongly assigned targets per-employee — the actual business flow is Admin → HR ("get this many admissions this month"), employees never get a target directly. Old `employee_targets` sheet/`appscript_employee_targets.gs`/old `targets_admin.html` retired (old sheet left in place, un-deleted, per rules.md #5 — a `deactivateLegacyEmployeeTargets()` helper flips its rows to `inactive` without touching the sheet itself). New: `hr_targets` sheet, new backend `appscript_hr_targets.gs` (`getHrTargets`/`assignHrTarget`/`updateHrTarget`, `setupHrTargetsSheet()`), rebuilt `targets_admin.html` (HR picker sourced from `getAdminUsers` filtered to `role === 'hr'`; read-only for HR viewers, full assign/edit for `super_admin` only — HR cannot self-assign). `achieved_target` still live-recomputed from `admission_leads` (status = `Admission Completed`), now matched by `assigned_to` against the target's `hr_id` instead of an employee's `emp_id`. `assigned_target` still falls back to the `MONTHLY_ADMISSION_TARGET` setting when left blank. Wiring patch: `hr_targets_wiring_patch.md` (exact `appscript_main.gs` + `appscript_generateid_fix.gs` edits — not yet applied to the live Apps Script project). ⚠️ **Blocking dependency:** `admission_leads.assigned_to` must actually be set to an HR's `user_id` for `achieved_target` to ever be non-zero — `admissions.html`'s "Assigned To" field needs to be checked/updated to list HR users instead of employees; not done this session (file wasn't provided). ⚠️ Not yet click-tested against the live Sheet (new schema, so the earlier "not yet click-tested" note applies fresh).
...
HR Targets live-sheet verification — apply `hr_targets_wiring_patch.md` to the live `appscript_main.gs`/`appscript_generateid_fix.gs`, run `setupHrTargetsSheet()`, fix `admissions.html`'s "Assigned To" field to list HR users (blocking — see §7.8 note), then click-test assign → live-recompute-on-admission-completion → edit/deactivate on the live Spreadsheet
