...
- [x] Daily task admin approval flow + routes (§7.6) — **shipped 2026-08-02**: new standalone page `daily_tasks_admin.html` (super_admin/branch_manager via sidebar; hr via a new "📝 Daily Tasks" tab on `hr_dashboard.html`, since HR is otherwise locked to `hr_dashboard.html`/`hr_recruitment.html` — `daily_tasks_admin.html` was added to `app.js`'s `HR_ALLOWED_PAGES`). Filterable list (approval status / employee / date range) + a review modal (Approve / Reject / Request Correction, remark required for the latter two) wired to `approveDailyTask`/`rejectDailyTask`/`requestCorrectionDailyTask`. Branch scoping for `branch_manager` is handled entirely server-side by the existing `getDailyTasks` filtering — no extra frontend logic needed. Daily Tasks module (§7.5–§7.7) is now fully end-to-end.
...
- [~] Candidate Management module → renamed in practice to **Admission Leads module**, per the resolved schema question (`architecture.md` §4) — genuinely separate pipeline from job-recruitment `candidates`, for student course-enrollment leads. Backend (`appscript_admissions.gs`) + frontend (`admissions.html`) written 2026-08-02. **Frontend wiring fully verified + complete** (2026-08-02 follow-up session): `hr_dashboard.html` navbar+quick-action, `app.js`'s `HR_ALLOWED_PAGES`, and `sidebar.html`'s nav link are all confirmed live/present — `admissions_frontend_patch.md` is fully applied. `admissions.html`'s HR-standalone/admin-sidebar dual-mode block also diff-checked against the real `daily_tasks_admin.html` and matches exactly. **⚠️ Backend wiring is the only remaining gap** — `appscript_admissions.gs` is still not pasted into the live Apps Script project, `appscript_main.gs`/`appscript_generateid_fix.gs` haven't been patched, and `setupAdmissionLeadsSheet()` hasn't been run, so the `admission_leads` sheet tab doesn't exist yet and every API call from `admissions.html` will currently fail. Follow `admissions_wiring_patch.md` to finish, then flip this to `[x]` (§7.10)
- [~] Soft-delete only for admission leads: `is_archived`/`archived_at`/`archived_by` fields, no delete route exposed to HR — **implemented in `appscript_admissions.gs`** (`archiveAdmissionLead`), same backend-wiring caveat as above applies — frontend (the Archive button in `admissions.html`) is ready and verified, only the backend route isn't live yet (§7.11)
