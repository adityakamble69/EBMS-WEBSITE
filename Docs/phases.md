...
- [~] Candidate Management module → renamed in practice to **Admission Leads module**, per the resolved schema question (`architecture.md` §4) — genuinely separate pipeline from job-recruitment `candidates`, for student course-enrollment leads. Backend (`appscript_admissions.gs`) + frontend (`admissions.html`) **written 2026-08-02** but **not yet pasted into the live Apps Script project / Google Sheet, and not yet linked from `hr_dashboard.html`/`sidebar.html`/`app.js`**. Follow `admissions_wiring_patch.md` + `admissions_frontend_patch.md` to finish, then run `setupAdmissionLeadsSheet()` once, then flip this to `[x]` (§7.10)
- [~] Soft-delete only for admission leads: `is_archived`/`archived_at`/`archived_by` fields, no delete route exposed to HR — **implemented in `appscript_admissions.gs`** (`archiveAdmissionLead`), same wiring caveat as above applies (§7.11)
